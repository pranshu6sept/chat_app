...
export const newMessage = asyncHandler(async(req,res) =>{
    ...
    const messagetext = await db.insert(messageTable).values({message,senderId,receiverId,conversationId,fileUrl,fileType }).returning();

    await db.update(conversationTable).set({message:message}).where(eq(conversationTable.id,conversationId))
    ...
    return res.status(201).json(
        new ApiResponse(200, messagetext, "message sent successfully")
    )
})
...
export const getMessages = asyncHandler(async(req,res) => {
     ...
     const messages = await db
                        .select().from(messageTable).where(eq(messageTable.conversationId,req.params.id))
